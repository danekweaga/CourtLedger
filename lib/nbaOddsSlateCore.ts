import { bdlFetch } from "./balldontlieClient";
...
export interface NbaOddsSlateResult {
...
export interface NbaOddsSlateKeys {
  oddsKey: string;
  bdlKey: string;
}
...
export async function buildNbaOddsSlate(keys: NbaOddsSlateKeys): Promise<NbaOddsSlateResult> {
  const oddsUrl =
    `${ODDS_BASE}/sports/basketball_nba/odds/?regions=us&markets=${MARKETS}&oddsFormat=american&apiKey=${encodeURIComponent(keys.oddsKey)}`;
  const oddsRes = await fetch(oddsUrl);
  const oddsRemaining = oddsRes.headers.get("x-requests-remaining");
  const oddsUsed = oddsRes.headers.get("x-requests-used");

  if (!oddsRes.ok) {
    const detail = (await oddsRes.text()).slice(0, 200);
    throw new OddsSlateUpstreamError(`Odds API ${oddsRes.status}`, detail);
  }
  ...
  const teamsJson = (await bdlFetch("/teams", keys.bdlKey)) as { data: BdlTeam[] };
  ...
  for (const row of scored) {
    ...
    try {
      attempts += 1;
      const pj = (await bdlFetch(`/players?search=${searchQ}&per_page=15`, keys.bdlKey)) as { data: BdlPlayer[] };
      players = pj.data ?? [];
    } catch {
      continue;
    }
    ...
    scenarios.push({
      ...
      role_shift_notes: "",
      notes: "Filled by nba-odds-slate (The Odds API + balldontlie). For entertainment only; verify lines on your book.",
    });
  }
...
export class OddsSlateUpstreamError extends Error {
  readonly status: number;
  readonly detail: string;

  constructor(message: string, detail: string) {
    super(message);
    this.name = "OddsSlateUpstreamError";
    this.status = 502;
    this.detail = detail;
  }
}
